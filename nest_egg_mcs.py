...
def read_to_list(file_name):
    """Open a file of data in percent, covert to decimal and return a list."""
...
print("\nNote: Inpute data should be in percent, not decimal.\n")
try:
...
# get user input and use dictionary for investment-type arguments.
...
# get user input
# Note: Might be better to define each input variable and then create a loop for the error handling.
...
while not withdrawal.isdigit():
    withdrawal = input("Invalid input. Input interger only: ")

min_years = default_input("Input minimum years in retirement: \n", '18')

while not min_years.isdigit():
    min_years = input("Invalid input. Input interger only: ")

most_likely_years = default_input("Input most-likely years in retirment: \n", '25')

while not most_likely_years.isdigit():
    most_likely_years = input("Invalid input. Input interger only: ")

max_years = default_input("input maximum years in retirment: \n", '40')

while not max_years.isdigit():
    max_years = input("Invalid input. Input interger only: ")

num_cases = default_input("Input number of cases to run: \n", '50000')

while not num_cases.isdigit():
    num_cases = input("Invalid input. Input interger only: ")
...
def bankrupt_prob(outcome, bankrupt_count):
    """Calculate the return chance of running out of money and other stats."""
    total = len(outcome)
...
def main():
    ...
    plt.ylabel('$ Remanining', fontsize=18)
